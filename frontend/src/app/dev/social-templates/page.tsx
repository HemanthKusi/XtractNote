...
export default function SocialTemplatesPage() {
  ...
  // Tone is on demand, like length: one generated by default, the rest when
  // asked for. Same hook, same race guard.
  const toneAxis = useOnDemand<Tone>("professional");
  const tone = toneAxis.value;
    ...
    <>
      <AppShell>
        ...
        <div className="mx-auto -mt-6 max-w-[1100px] px-6 pb-24">
          <Direction
            tone={tone}
            setTone={toneAxis.request}
            platform={platform}
            toneReady={toneAxis.ready}
            toneGenerating={toneAxis.generating}
          >
            {platform === "x-thread" ? (
              <XThread key={tone} threads={X_THREAD[tone]} />
            ) : (
